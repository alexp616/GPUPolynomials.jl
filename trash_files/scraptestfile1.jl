...
function toString(c::ComplexF64)
    return "$(real(c)) + $(imag(c))im"
end

function toString(c::ComplexF32)
    return "$(real(c)) + $(imag(c))im"
end
    ...
    for i in 1:log2n
        ...
        for j in 0:m2-1
            for k in j:m:n-1
                ...
                println("k: $k, t: $(round(t)), u: $(round(u)), theta: $theta")
                result[k + 1] = u + t
                result[k + m2 + 1] = u - t
            end
            theta *= theta_m
...
function gpuDFTKernel!(result, m2, theta_m, magic)
    ...
    @cuprintln("k: $k, t: $(real(t)) + $(imag(t))im, u: $(real(u)) + $(imag(u))im, theta: $(real(theta)) + $(imag(theta))im")
    result[k + 1] = u + t
    result[k + m2 + 1] = u - t
    return 
end
...
# println("-----------------start------------------")
println(iterativeDFT(polynomial1))
println(gpuDFT(polynomial1))
# println(recursiveDFT(polynomial1))
