...
function cpu_intt(vec, prime, npru, len, log2length, pregen_butterfly = nothing)
...
function cpu_pow(p1::Array{Int, 1}, pow; prime, npru, len = -1, pregen_butterfly = nothing)
    if len == -1
    ...
    if pregen_butterfly === nothing
        pregen_butterfly = Array(generate_butterfly_permutations(len))
    end
