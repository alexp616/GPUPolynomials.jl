...
function GPUpolynomialPow(p::CuArray, n::Int)
    ...
    for i in 1:length(bitarr)-1
        if i == 1
            result = GPUMultiply(result, temp)
        temp = GPUSquare(temp)
        end
    end
    if bitarr[end] == 1
...
function GPUSquare(p::CuArray)
    # TODO Try putting everything into one method to see if significant change in speed
    finalLength = 2 * length(p) - 1
    n = Int(2^ceil(log2(finalLength)))
    log2n = UInt32(log2(n));

    # TODO Decide whether throwing an if statement in the DFTs is faster or
    # if all of this memory copying is necessary.
    cudap = vcat(copy(p), zeros(Int, n - length(p)))

    cudap = GPUDFT(cudap, n, log2n)
    println("this ran")
    return GPUIDFT(cudap .* cudap, n, log2n)[1:finalLength]
end

p = CuArray([1, 1])
println(GPUpolynomialPow(p, 4))

# p = [1, 2, 3, 4, 5, 6]
# println()
# @btime GPUpolynomialPow(p, 30)
